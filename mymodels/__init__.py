...
files = glob.glob('mymodels/[!_]*.py')
for file in files:
	# try:
	if file.split('.')[0].replace(os.path.sep, '.')=="mymodels.patchcore":
		print(file.split('.')[0].replace(os.path.sep, '.'))
	model_lib = importlib.import_module(file.split('.')[0].replace(os.path.sep, '.'))
	# except:
	# 	print(f"file:{file},error")#:{traceback.print_exc()}
def get_TRAINER(name):
    return TRAINER.get_module(name)
...
def get_model(cfg_model):
	...
	model_fn = TRAINER.get_module(model_name)
